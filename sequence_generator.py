...
def scalefree_sequence(N, mean, seqtype):
    ...
    while tol > 0.2:
        ...
        # check if the average of the total-degree list is close to the network
        # mean degree (d). Tolerance=0.5 deviation from d.
        if tol > 0.2:
...
def geometric_sequence(N, mean, seqtype):
    ...
    while (abs(mean - calc_mean)) > 0.05:
        if (mean - calc_mean) > 0.05:
            ...
            avg -= 0.1
        x = 1.0 / avg
        seq = [(math.log(np.random. random()) / math.log(1 - x))
               for i in range(N)]
        seq = [(int(round(x))) for x in seq]
        # compute avg degree of generated degree sequence
        calc_mean = sum(seq) / (1.0 * len(seq))
