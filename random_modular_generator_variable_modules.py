...
# enter the degree distribution, modularity, total network size, number of
# modules, and the mean degree


def generate_modular_networks(
        N, sfunction, modfunction, Q, m, avg_degree, verbose=False, **kwds):
    ...
    while not is_valid_module_seq:
        ...
        # wd_hat = average within-degree when module sizes are kept constant.
        # This value is used to fix minimum size of modules
        wd_hat = avg_degree * (Q + (1. / m))
        scale += 0.5
        mod_nodes = assign_module_membership(
            scale, N, nc, m, wd_hat, modfunction)
        ...
        tol = 0.1 * avg_degree / \
            (1.0 * (1 - sum([(num / (1.0 * N))**2 for num in mod_sizes])))
        # print ("tol=="), tol
        ...
        wd1 = avg_degree * \
            (Q + (sum([(num / (1.0 * N))**2 for num in mod_sizes])))
        wd = round(wd1, 2)
        ...
        while connect_trial >= 10 or outedge_graphical == False or graph_connected == False:
            ...
            degree_list = create_total_degree_sequence(
                N, sfunction, avg_degree, mod_nodes, 0.01 * tol, max_tries=1000, **kwds)

            # assigns within-degree to each node
            print("generating indegree lists......")
            indegree_list = create_indegree_sequence(
                N, m, sfunction, mod_nodes, wd, degree_list, tol, **kwds)
            ...
            outdegree_list = create_outdegree_sequence(
                degree_list, indegree_list)
            ...
            if outedge_graphical == True:
                ...
                connect_trial = connect_out_nodes(
                    G, m, mod_nodes, outdegree_list, connect_trial, indegree_list)

                # connect within-module edges only when between-module edges
                # are connected.
                if len(G.edges()) > 1:
                    ...
                    connect_in_nodes(
                        G, m, mod_nodes, indegree_list, outdegree_list)
...
def assign_module_membership(scale, N, nc, m, wd_hat, modfunction):
    ...
    valid_seq = False
    while not valid_seq:
        # generate m-1  number of random nos.
        trialseq = modfunction(m - 1, nc, seqtype="modulesize")
        seq = [max(min_size, s) for s in trialseq]
        # so that the last module is at least size=min_size
        valid_seq = (N - sum(seq)) >= min_size
...
def create_total_degree_sequence(
        n, sfunction, avg_degree, mod_nodes, tolerance, max_tries=2000, **kwds):
    """
    ...
    for mod in range(len(mod_nodes.keys())):
        ...
        while tol > tolerance or (not is_valid_seq) or (tries > max_tries):
            trialseq = sfunction(
                len(mod_nodes[mod]), avg_degree, seqtype="degree")
...
def create_indegree_sequence(
        n, m, sfunction, mod_nodes, wd, degree_list, tolerance, **kwds):
    """
    ...
    while ((not is_valid_seq) or (not is_valid_indegree) or (
            not is_valid_module_size) or (not is_valid_outdegree)):

        indegree_seq = sfunction(n, wd, seqtype="indegree")
        indegree_sort = list(np.sort(indegree_seq))
        degree_sort = list(np.sort(degree_list))
        is_valid_indegree = all(
            [indegree_sort[i] <= degree_sort[i] for i in range(n)]) == True
        mod_sizes = [len(mod_nodes[x]) for x in mod_nodes.keys()]
        ...
        if is_valid_indegree and is_valid_seq and is_valid_module_size:
            ...
            indegree_list = sort_inedge(
                indegree_sort, degree_sort, degree_list, n)
            # if network has two modules then sum(outdegree) for module 1 has
            # to be equal to sum(outdegree) of module 2
            if m == 2:
            ...
            for module in mod_nodes:
                ...
                while (sum(seq) % 2) != 0:
                    ...
                    node_add_degree = rnd.choice(
                        [i for i in mod_nodes[module]])
                    # ensure that wd<=d and wd< (module size -1)after adding a
                    # within-degree to the node
                    if indegree_list[node_add_degree] < degree_list[node_add_degree] and indegree_list[node_add_degree] < len(
                            mod_nodes[module]):
                ...
                tol = abs(wd - (sum(seq) / (1.0 * len(seq))))
...
def connect_in_nodes(G, m, mod_nodes, indegree_list, outdegree_list):
    ...
    while a < m:
        ...
        # sorts within-degrees in descending order keeping the  node identity
        # intact
        ...
        while node_edge:
            ...
            # choose node with highest within-degree
            deg1, node1 = node_edge[0]
            if deg1 > 0:
                # connect the stubs of the node(i) to the next "wd(i)" nodes in
                # the sorted list.
                for num in range(1, 1 + deg1):
                    ...
                    node_edge = [(x - 1, y) if y == node2 else (x, y)
                                 for x, y in node_edge]
...
def connect_out_nodes(G, m, mod_nodes, outdegree_list,
                      connect_trial, indegree_list):
    ...
    while is_valid_connection == False and connect_trial < 10:
        ...
        while outnodelist:
            ...
            # removes outnodes with zero outedges
            outnodelist = [(x, y, z) for x, y, z in outnodelist if y != 0]

            # select module with the highest between-degree = hfm
            outmod_tot = [(sum([y for x, y, z in outnodelist if z == a]), a)
                          for a in range(m)]
            outmod_tot.sort(reverse=True)
            hfm = outmod_tot[0][1]

            # Select node (=node1) in module=hfm which has the highest
            # between-degree
            possible_node1 = [(x, y) for x, y, z in outnodelist if z == hfm]
            possible_node1 = sorted(
                possible_node1,
                key=lambda x: x[1],
                reverse=True)
            ...
            for degrees in range(deg1):
                if is_valid_connection == False:
                    break
                 # criteria (a) and (b)
                node_exclude = set(G.neighbors(node1)).union(
                    set(mod_nodes[hfm]))
                possible_node2 = [
                    (x, y, z) for x, y, z in outnodelist if x not in node_exclude]
                # list of possible nodes that node1 can connect to.

                possible_node2 = [(x, y) for x, y, z in possible_node2]

                # terminate if there are no possible nodes left for node 1 to
                # connenct to.
                ...
                # Avoids formation of disconnected graphs
                    trial = 0
                    # preference to nodes with indegree>0 to connect to nodes
                    # to indegree= 0
                    if indegree_list[node1] != 0 and [indegree_list[node]
                                                      for (node, deg) in possible_node2].count(0) > 1:
                        possible_node2 = [
                            (node, deg) for node, deg in possible_node2 if indegree_list[node] == 0]
                    while not is_isolates_avoided:
                        ...
                        if is_isolates_avoided:
                            trial = 0

                        # terminate if the attempt of finding possible nodes
                        # exceed 10000
                        if trial == 50000:
                            is_valid_connection, connect_trial = remove_outedges(
                                G, connect_trial)
                            break

                # on termination remove all the between-edges added and try
                # again
                else:
                    is_valid_connection, connect_trial = remove_outedges(
                        G, connect_trial)
                ...
                if is_valid_connection == True:
                    ...
                    outnodelist = [
                        (x, y, z) if x != node2 else (
                            x, y - 1, z) for x, y, z in outnodelist]

            # remove node if the between-degree (outstub) hits zero
            outnodelist = [(x, y, z)
                           for x, y, z in outnodelist if y > 0 and x != node1]
...
def is_graphical(edgelist, mod_nodes, m):
    ...
    if m == 2:
        ...
        for j in range(1, n + 1):
            lhs.append(sum([s[i] for i in range(1, j + 1)]) -
                       (b * j * (j - 1)))
...
def randomize_graph_outedges(G, mod_nodes, indegree_list, outdegree_list):
    ...
    size = G.size()
    double_edge_swap_outedges(
        G,
        mod_nodes,
        indegree_list,
        outdegree_list,
        nswap=5 * size,
        max_tries=10000 * size)

#############################################################################


def double_edge_swap_outedges(
        G, mod_nodes, indegree_list, outdegree_list, nswap, max_tries):
...
def connect_module_graph(G, outdegree_list):
    ...
    cc_tot = list(nx.connected_components(
        # cc returns the connected components of G as lists cc[0], cc[1], etc.
        G))
    ...
    while isolated_comp_count > 0:  # while G is not connected, reduce number of components
        ...
        node2 = rnd.choice(outedge_comp[rnd.choice(
            [x for x in range(outedge_comp_count)])])
...
def adjust_indegree_seq(mod_nodes, indegree_seq):
    ...
    mod_size = sorted([(len(mod_nodes[s]), s) for s in mod_nodes])
    adjust_indegree_seq = {}
    for mods in mod_size[:-1]:
        ...
        while not (max_deg_valid) or not (total_deg_valid):
            ...
            adjust_indegree_seq[mods[1]] = [indegree_topop.pop()
                                            for nodes in mod_nodes[mods[1]]]
...
def test_modularity_variable_mod(G, mod_nodes, verbose):
    ...
    for modules in range(0, mods):
        ...
        for node in mod_nodes[modules]:
            ...
            # set of neighbors present in the same module
    ...
    Q_list = [((wd_bar[i] * s[i]) / (1.0 * avg_deg * N)) -
              (s[i] / (1.0 * N))**2 for i in range(mods)]
    Q = sum(Q_list)
    print("Summary...")
    ...
    if verbose:
        for modules in range(0, mods):
            print(f"Module #{modules} has node ids = {mod_nodes[modules]}")
...
if __name__ == "__main__":
    ...
    # Enter the network size(N), average network degree (d), total modules in
    # the network (m), and modularity (Q)
    N = 2000
    d = 10
    m = 10
    Q = 0.4
    ...
    # four well known degree distribution found in biological networks -
    # scalefree, geometric, poisson and regular distribution
    ...
    # in it's simplest form speicify module size tp be regular which implies
    # that all modules are of equal size
    ...
    try:
        ...
        # The graphml file can be uploaded in Gephi (http://gephi.github.io/)
